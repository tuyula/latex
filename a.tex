...
\begin{tikzpicture}[node distance=1.5cm, auto]
  ...
  \node (Instr) [above left=of Control] {指令};
  \node (ReadData) [below=of Instr] {读取数据};

  \node (WriteData) [below right=of MemWrite] {写入数据};
  \node (WriteReg) [below right=of RegWrite] {写寄存器};

  \node (ALUResult) [right=of ALUSrc] {ALU结果};
